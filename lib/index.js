...
export default fp(
  ...
  {
    name: 'export-federation-data',
    dependencies: ['mercurius']
  }
)

function updateIsExtension(type, value) {
  if (type.name.startsWith('_') || type.kind === 'SCALAR') return
  //   console.log(JSON.stringify(value.schema._typeMap[type.name]?.extensionASTNodes, null, 2));
  if (
